...
import { downloadMonthlyStaffAttendanceReportAdmin, downloadMonthlyDriverAttendanceReportAdmin } from "@/lib/admin-api";
...
export default function AdminAttendanceReportPage() {
  ...
  async function handleDownload(kind: "staff" | "drivers") {
    ...
    setError(null);
    try {
      if (kind === "staff") await downloadMonthlyStaffAttendanceReportAdmin(year, month);
      else await downloadMonthlyDriverAttendanceReportAdmin(year, month);
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-slate-50">
        ...
        <main className="mx-auto max-w-2xl px-6 py-10">
          <p className="text-sm text-red-700">This page is only available to the Commissioner.</p>
        </main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-2xl px-6 py-10">
        <h1 className="mb-1 text-2xl font-semibold text-slate-900">Field Staff Monthly Attendance Report</h1>
